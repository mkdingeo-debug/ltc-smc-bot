...
"""
================================================================================
 NATHANIEL RIDGE — RIDGECREST CRYPTO | ANALISTA SMC/ICT MULTI-CRIPTO (Bybit)
================================================================================
...
# ATR: mide la volatilidad real de cada cripto (LTC no se mueve como BTC) y
# se usa para el colchón de invalidación, en vez de un % fijo del rango que
# trataba igual a todas las monedas de SYMBOLS.
...
# Un BOS/CHoCH con volumen real de Bybit por encima del promedio reciente es
# una confirmación institucional más fiable que uno con volumen plano.
BREAK_VOLUME_CONFIRM_MULT = float(os.environ.get("BREAK_VOLUME_CONFIRM_MULT", "1.2"))

# Etiqueta opcional para distinguir despliegues del mismo código (ej. bot de
# pruebas vs. bot de producción) sin tener que tocar el código cada vez.
# Configúralo como variable de entorno DEPLOY_LABEL en Railway (ej. "PRUEBAS").
...
def _is_break_volume_confirmed(df: pd.DataFrame, index: int, lookback: int = VOLUME_LOOKBACK,
    ...
    """Un BOS/CHoCH acompañado de volumen por encima del promedio reciente es
    una confirmación institucional mucho más fiable que uno con volumen plano."""
    start = max(0, index - lookback)
    prior = df["volume"].iloc[start:index]
    if prior.empty or prior.mean() == 0:
        return True  # sin referencia suficiente, no penalizamos
...
def calculate_atr(df: pd.DataFrame, period: int = ATR_PERIOD) -> float:
    """Average True Range: mide la volatilidad real de cada cripto. LTC y
    BTC no se mueven igual — el ATR adapta el colchón de invalidación a
    cada símbolo de SYMBOLS en vez de usar un % fijo del rango para todos."""
...
class TradePlan:
    ...
    rr2: float
    rr3: float
    tp1_source: str  # "liquidez" o "límite del rango" — de dónde sale el TP1, para trazabilidad


MAX_TP1_DISTANCE_MULT = 2.0  # TP1 no puede estar a más de N veces el tamaño del rango
...
def compute_trade_plan(bias: str, price: float, pd_info: Optional[PremiumDiscount],
    ...
    # Verificación de consistencia direccional: el tramo (leg) más reciente
    # usado para Premium/Discount debe apuntar en la MISMA dirección que el
    # sesgo oficial (BOS/CHoCH). Si el sesgo es alcista pero el tramo más
    # reciente en realidad bajó (o viceversa), la zona OTE calculada queda
    # del lado equivocado del precio y el plan sería engañoso — mejor no
    # generarlo, aunque el R:R diera bien por pura coincidencia numérica.
    ...
    # ajustado — cada cripto de SYMBOLS recibe un margen acorde a su
    # volatilidad real (LTC no se mueve como BTC).
    ...
    if bias == "alcista":
        ...
        # Solo usamos la zona de liquidez como TP1 si está a una distancia
        # razonable — si el único nivel conocido está muy lejos (ej. de hace
        # semanas), es poco representativo como primer objetivo y da un
        # R:R engañosamente alto. Preferimos el límite del rango, que sí es
        # visible y verificable en el propio reporte.
...
def generate_signal(symbol: str, df: pd.DataFrame, events: List[StructureEvent], zones: List[Zone]) -> SignalReport:
    ...
    # acompañado de volumen real de Bybit por encima del promedio — un
    # BOS/CHoCH "silencioso" es una huella institucional menos convincente.
    volume_note = ""
...
def render_template_report(report: SignalReport, interval: str) -> str:
    ...
    if report.trade_plan:
        ...
        L.append(f"  <b>Entrada (POI):</b> {tp.entry_low:.4f} — {tp.entry_high:.4f}")
        L.append(f"  <b>Invalidación (SL):</b> {tp.stop_loss:.4f}")
        L.append(f"  <b>TP1:</b> {tp.take_profit_1:.4f}  (<b>R:R</b> {tp.rr1:.2f}) — fuente: {tp.tp1_source}")
        L.append(f"  <b>TP2:</b> {tp.take_profit_2:.4f}  (<b>R:R</b> {tp.rr2:.2f})")
        L.append(f"  <b>TP3:</b> {tp.take_profit_3:.4f}  (<b>R:R</b> {tp.rr3:.2f})")
        ...
        L.append("")
        L.append(f"<b>5) PLAN DE TRADING HIPOTÉTICO</b> {'🟢' if report.bias == 'alcista' else '🔴'}")
