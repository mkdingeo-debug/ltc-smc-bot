...
def compute_premium_discount(df: pd.DataFrame, swings: List[SwingPoint]) -> Optional[PremiumDiscount]:
    ...
    zone = "descuento (discount)" if pct < 0.45 else "premium" if pct > 0.55 else "equilibrio (50%)"
...
def generate_signal(symbol: str, df: pd.DataFrame, events: List[StructureEvent], zones: List[Zone]) -> SignalReport:
    ...
    liquidity_zones = [z for z in zones if z.kind.startswith("liquidity_")]
    ob_zones = [z for z in zones if "order_block" in z.kind]
    fvg_zones = [z for z in zones if z.kind.startswith("fvg_")]

    invalidation = last_event.price if last_event else None
    pd_info = compute_premium_discount(df, find_swing_points(df))
    trade_plan = compute_trade_plan(bias, price, pd_info, zones)
    ...
    return SignalReport(
        ...
        ob_zones=[z for z in ob_zones if not z.mitigated][:3],
...
def render_template_report(report: SignalReport, interval: str) -> str:
    ...
    if report.premium_discount:
        ...
        L.append(f"  Rango activo: {pd_i.range_low:.4f} — {pd_i.range_high:.4f}")
        L.append(f"  Precio está en: {pd_i.zone.upper()} ({pd_i.pct_in_range*100:.0f}% del rango)")
