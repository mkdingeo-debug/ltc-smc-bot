...
def compute_premium_discount(df: pd.DataFrame, swings: List[SwingPoint]) -> Optional[PremiumDiscount]:
    ...
    if pct > 1.0:
        zone = f"EXTENSIÓN ALCISTA — precio {((pct - 1) * 100):.0f}% por encima del último swing high confirmado"
    elif pct < 0.0:
        zone = f"EXTENSIÓN BAJISTA — precio {(-pct * 100):.0f}% por debajo del último swing low confirmado"
    elif pct < 0.45:
